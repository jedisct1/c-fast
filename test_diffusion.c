...
test_single_bit_flip(fast_context_t *ctx, size_t word_length, diffusion_stats_t *stats)
{
    ...
    double variance =
        (sum_squared / test_count) - (stats->avg_bit_flip_ratio * stats->avg_bit_flip_ratio);
...
main(int argc, char *argv[])
{
    ...
    for (size_t wl_idx = 0; wl_idx < num_word_lengths; wl_idx++) {
        ...
        printf("Parameters: layers=%u, w=%u, w'=%u, sboxes=%u\n", params.num_layers,
               params.branch_dist1, params.branch_dist2, params.sbox_count);
        ...
        printf("Tests passed (>40%% threshold): %d/%d (%.1f%%)\n", stats.passed_tests,
               stats.total_tests, (double) stats.passed_tests / stats.total_tests * 100);
