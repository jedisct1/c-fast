...
static const uint8_t BENCH_TWEAK[BENCH_TWEAK_LEN] = {
    0xBA, 0xDC, 0x0F, 0xFE, 0xED, 0x1A, 0x71, 0x0E
};
...
static void
run_benchmark(fast_context_t *ctx, size_t data_size, int iterations, benchmark_result_t *result,
              uint32_t radix)
{
...
benchmark_different_parameters()
{
    ...
    printf("%-10s %-10s %-10s %-15s %-15s %-15s %-15s\n", "Radix", "WordLen", "Layers",
           "Encrypt(µs)", "Decrypt(µs)", "Enc MB/s", "Dec MB/s");
    printf(
        "------------------------------------------------------------------------------------------"
        "---------------\n");
    ...
    struct {
        ...
        { 64, 16 }, { 10, 8 }, { 10, 32 }, { 10, 64 }, { 10, 128 },
    };

    for (size_t i = 0; i < sizeof(test_configs) / sizeof(test_configs[0]); i++) {
        fast_params_t params;
        if (calculate_recommended_params(&params, test_configs[i].radix,
                                         test_configs[i].word_length) != 0) {
            printf("%-10u %-10u Failed to calculate parameters\n", test_configs[i].radix,
                   test_configs[i].word_length);
            continue;
        }
        ...
        run_benchmark(ctx, test_configs[i].word_length, BENCHMARK_ITERATIONS, &result,
                      params.radix);
...
benchmark_data_sizes()
{
    ...
    printf("%-15s %-15s %-15s %-15s %-15s\n", "Data Size", "Encrypt(µs)", "Decrypt(µs)", "Enc MB/s",
           "Dec MB/s");
    printf("---------------------------------------------------------------------------------\n");
